 ...
 * MvcModel
 * 
 * A database utility wrapper for MDB2.  An MvcModel class represents one record in a table.
 */
abstract class MvcModel extends MDB2 {
	 ...
	 * 
	 * @param string $aClassFile The filesystem path to the instantiated MvcModel class.
...
#showDebug( $sql ) ;
	 ...
	 * objectFactory
	 *
	 * Static method to create an MvcModel object determined
	 * from an array of &quot;fieldname=value&quot; elements.
	 *
	 * @param string $className The type of MvcModel object to instantiate.
	 ...
	 * collectionFactory
	 *
	 * Static method to create an array of MvcModel objects determined
	 * from an array of &quot;fieldname=value&quot; elements.
	 *
	 * @param string $className The type of MvcModel object to instantiate.
	 * @param array $params An array of &quot;fieldname=value&quot; elements.
	 ...
	 *
	 * @param $className The name of the MvcModel object to use.
	 ...
	 *
	 */
	protected function validateField( $aField ) {
		$isValid = false ;

		$fieldType = $this->schema['field_definitions'][$aField] ;
		switch( $fieldType ) {
		case DB_DATATYPE_STRING_BASIC :
		case DB_DATATYPE_STRING_EMAIL :
		case DB_DATATYPE_DATE :
	 ...
	 * isValid
	 *
	 * Validate the MvcModel object's data.
	 *
	 */
	protected function isValid() {
		...
		foreach( $this->schema['fields'] as $nextField ) {
			$fieldIsValid = $this->validateField( $nextField ) ;

			if( $fieldIsValid ) { $isValid = true ; }
			else {
				$isValid = false ;
				break ;
			}
	 ...
	 *
	 */ 
	protected static function emptyNotZero( $aValue ) {
		if( empty( $aValue )) {
